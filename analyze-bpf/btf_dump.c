...
static int btf_dump_order_type(struct btf_dump *d, __u32 id, bool through_ptr)
{
	...
	if (tstate->order_state == ORDERING) {
		...
		if (btf_is_composite(t) && through_ptr && t->name_off != 0)
			return 0;
		printf("unsatisfiable type cycle, id:[%u]\n", id);
		return -ELOOP;
	}
	...
	if (tstate->emit_state == EMITTING) {
		...
		case BTF_KIND_UNION:
			...
			if (id == cont_id)
				return;
			if (t->name_off == 0) {
				printf("anonymous struct/union loop, id:[%u]\n",
					id);
				return;
			}
				    ...
				    const char *fname, int lvl)
{
	...
	for (;;) {
		...
		if (err < 0) {
			 ...
			 */
			printf("not enough memory for decl stack:%d", err);
...
skip_mod:
		...
		case BTF_KIND_FLOAT:
			goto done;
		default:
			printf("unexpected type in decl chain, kind:%u, id:[%u]\n",
				btf_kind(t), id);
			goto done;
		}
	}
				       ...
				       __u64 *value)
{
	...
	if (t->size > 8) {
		printf("unexpected bitfield size %d\n", t->size);
		return -EINVAL;
	}
					 ...
					 const void *data)
{
	...
	if (nr_bytes < 1 || nr_bytes > 16) {
		printf("unexpected size %d for id [%u]\n", nr_bytes, id);
		return -EINVAL;
	}
...
# error "Unrecognized __BYTE_ORDER__"
	...
	case 1:
		...
		break;
	default:
		printf("unexpected sz %d for id [%u]\n", sz, type_id);
		return -EINVAL;
	}
	return 0;
}
			       ...
			       const void *data)
{
	...
	if (elem_size <= 0) {
		printf("unexpected elem size %zd for array type [%u]\n",
				   ...
				   __s64 *value)
{
	...
	case 1:
		*value = is_signed ? *(__s8 *)data : *(__u8 *)data;
		return 0;
	default:
		printf("unexpected size %d for enum, id:[%u]\n", t->size, id);
		return -EINVAL;
	}
}
					     ...
					     __u8 bit_sz)
{
	...
	if (size < 0 || size >= INT_MAX) {
		printf("unexpected size [%zu] for id [%u]\n",
			(size_t)size, id);
		return -EINVAL;
	}
	...
	if (!t) {
		printf("unexpected error skipping mods/typedefs for id [%u]\n",
			id);
		return -EINVAL;
	}
				   ...
				   __u8 bit_sz)
{
	...
	case BTF_KIND_DATASEC:
		...
		break;
	default:
		printf("unexpected kind [%u] for id [%u]\n",
